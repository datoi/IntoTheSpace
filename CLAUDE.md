...
6. When something is broken, find the root cause instead of patching symptoms,
   and verify the fix by running the test suite (`npm test`) and the typecheck
   (`npx tsc --noEmit`). A behavioural fix should come with a test that fails
   without it — several bugs here were regressions of things nothing covered.
...
- `npm test` — Jest (jest-expo + @testing-library/react-native). Run it before
  calling anything done; the suite covers the game loop, save/migration,
  economy and quest rollover, and it does catch real regressions.

## Asset pipeline

Source art lives in `art-src/` and NEVER in `assets/` — `assetBundlePatterns`
in app.json is a list of single-level globs, so anything under `assets/` ships
and anything nested inside it silently does not. Generated by:

- `node scripts/bake-bg-dim.mjs --write` — bakes BG_DIM into the backgrounds
- `node scripts/make-explosion-sheets.mjs` — packs explosion frames into sheets
- `node scripts/make-shot-sfx.mjs` — synthesises the gun sounds
- `node scripts/make-ui-sfx.mjs` — synthesises the menu/interface sounds

`src/game/__tests__/assetBundle.test.ts` guards that boundary in both
directions. Keep `assets/` flat.
